...
auth.onAuthStateChanged(user => {
	...
	if(user.email || localStorage.getItem('emailGuy')) {
		emailP.innerHTML = `
			Email Invoice To: <br>
			<span id="uida">${localStorage.getItem('emailGuy')}</span>.`;
...
function phoneShow2() {
	...
	if(auth.currentUser.email || localStorage.getItem('emailGuy')) {
		...
		heyName1.innerHTML = ` A <span>code</span> will be sent to your <br> <span>phone</span> number. `;
	}
...
function emailShow2() {
	...
	if(auth.currentUser.phoneNumber) {
		...
		heyName1.innerHTML = ` A <span>link</span> will be sent to your <br> <span>email</span> inbox. `;
	}
